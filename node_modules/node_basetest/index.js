...
function HASHELPER(one, two) {
    ...
    if (isArr) {
        for (let i = 0; i < isArr.length; i++) {
            stringed = stringed + isArr[i]
        }
        includes = S(stringed).contains(two);
    } else if (typeof one === "string") {
        includes = S(one).contains(two);
    } else if (typeof one === "object") {
        let stringedObject = JSON.stringify(one)
        includes = S(stringedObject).contains(two);
    } else {
...
module.exports = { T, F, HASHELPER, HAS, COUNTHELPER, COUNT, ATLEASTHELPER, ATLEAST, DOESNOTHAVEHELPER, DOESNOTHAVE, ISTYPEOF, ENDSWITHHELPER, ENDSWITH, STARTSWITHHELPER, STARTSWITH, TRUNCATE, WRAPHTML, ISLESS, ISLESSHELPER };